...
Rerun the selected ticket acceptance checks from the clean candidate, including required native/wire/Linux/live evidence. App-only changes do not require all unchanged runtime, SDK, renderer, example or benchmark suites. Documentation-only changes need relevant validators. Reuse recorded baseline evidence only after checking that the relevant source, dependency pins, test configuration and environment remain applicable; cite its revision and results separately from your own runs.
