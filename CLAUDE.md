...
- **Swift** — `client-macos/Protocol/srui.pb.swift` and `client-macos/SemanticModel/RegistryTables.swift` are **committed generated files**. The shared scheduling cycle is generated into `client-macos/LogicalChannelScheduling/Sources/LogicalChannelScheduling/LogicalChannelPolicy.generated.swift`. Regenerate with `./protocol/generate_proto.sh` (runs `protoc` + `protoc-gen-swift`, then `protocol/generate_swift_registry.py`, `protocol/generate_logical_channel_policy.py` and `protocol/generate_conformance_matrix.py`). CI re-runs it and fails on any `git diff`.
- **Conformance matrix** — `protocol/conformance-vectors/suites/{02-widget-semantics,05-semantic-input,12-toolkit-mapping}/*.generated.json` are committed generated files derived from `registry.yaml` by `protocol/generate_conformance_matrix.py`. Node `tier`, `category` and the `emits` matrix are registry-owned and must never be restated by hand in a fixture. The AppKit mapping table is the exception and lives in the generator, because §22.4 makes native mappings informative rather than protocol source of truth.
