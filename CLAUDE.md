...
- **Swift** — `client-macos/Protocol/srui.pb.swift` and `client-macos/SemanticModel/RegistryTables.swift` are **committed generated files**. Regenerate with `./protocol/generate_proto.sh` (runs `protoc` + `protoc-gen-swift`, then `protocol/generate_swift_registry.py`). CI re-runs it and fails on any `git diff`.
