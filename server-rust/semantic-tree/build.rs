...
fn main() {
    ...
    let content = fs::read_to_string(&registry_path).unwrap_or_else(|e| {
        panic!(
            "Failed to read registry.yaml at {}: {}",
            registry_path.display(),
            e
        )
    });

    let registry: RegistrySchema = serde_yaml::from_str(&content).unwrap_or_else(|e| {
        panic!(
            "Failed to parse registry.yaml at {}: {}",
            registry_path.display(),
            e
        )
    });
    ...
    assert_eq!(
        registry.node_types.len(),
        27,
        "Expected 27 standard node types, parsed {}",
        registry.node_types.len()
    );
    assert_eq!(
        registry.properties.len(),
        30,
        "Expected 30 standard properties, parsed {}",
        registry.properties.len()
    );
    assert_eq!(
        registry.enums.len(),
        12,
        "Expected 12 standard enums, parsed {}",
        registry.enums.len()
    );
    assert_eq!(
        registry.events.len(),
        11,
        "Expected 11 standard events, parsed {}",
        registry.events.len()
    );
    assert_eq!(
        registry.operations.len(),
        13,
        "Expected 13 standard operations, parsed {}",
        registry.operations.len()
    );
    ...
    code.push_str(
        "// Generated by semantic-tree/build.rs using serde_yaml. Do not edit manually.\n\n",
    );
    ...
    code.push_str(
        "    // Generated Standard Node Type constants from protocol/registry.yaml (§7.2)\n",
    );
    for item in &registry.node_types {
        let const_name = to_upper_snake_case(&item.name);
        code.push_str(&format!(
            "    pub const {}: Self = Self::standard({});\n",
            const_name, item.id
        ));
        if item.name == "RichText" {
            code.push_str(&format!(
                "    pub const RICHTEXT: Self = Self::standard({});\n",
                item.id
            ));
        }
    }
    ...
    code.push_str(
        "    // Generated Standard Property constants from protocol/registry.yaml (§7.4)\n",
    );
    for item in &registry.properties {
        let const_name = to_upper_snake_case(&item.name);
        code.push_str(&format!(
    ...
    code.push_str(
        "    // Generated Standard Enum value constants from protocol/registry.yaml (§7.5)\n",
    );
    ...
    code.push_str(
        "pub fn lookup_standard_enum_value(enum_id: u32, value_name: &str) -> Option<u32> {\n",
    );
    ...
    code.push_str(
        "pub fn standard_enum_value_name(enum_id: u32, value_id: u32) -> Option<&'static str> {\n",
    );
    ...
    fs::write(&dest_path, code)
        .unwrap_or_else(|e| panic!("Failed to write registry_tables.rs: {}", e));
}
