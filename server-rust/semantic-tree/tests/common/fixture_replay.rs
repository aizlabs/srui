//! Declarative fixture replay engine shared by the §32 vector suites.
//!
//! Implements: §12.1 (atomic transactions), §13 (operations), §26 (limits), §32.1, §32.3.
//!
//! Suite 1 (`state_machine_conformance_test.rs`) and suite 3
//! (`conformance_semantic_not_paint_test.rs`) both replay fixtures in this format. The engine
//! lives here so a vector relocated between suites is still *executed* rather than merely parsed
//! by whichever runner happens to own it.
...
pub fn replay_fixture(path: &Path) {
    let file_name = path
        .file_name()
        .and_then(|f| f.to_str())
        .unwrap_or("<unknown>");
